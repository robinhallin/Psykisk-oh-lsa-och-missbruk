# Studieportal V6.2 – Polisiärt arbete vid psykisk ohälsa

V6.2 bygger vidare på de 16 fördjupade kursmodulerna och lägger till två stora träningsspår: **tentamensinspirerad träning** och ett **visuellt/adaptivt träningslabb**.


## V6.2 – buggfix för tentamensmotorn

V6 hade en missad hjälpfunktion (`setupSegment`) i `app.js`. Det gjorde att Tentamen-sidan hann renderas men JavaScript avbröts innan klickhändelserna kopplades, vilket gjorde inställningarna och **Starta testet** oklickbara. V6.2 lägger tillbaka funktionen och återställer klickbarheten för frågeantal, läge, tid och startknapp.
...
Portalen visar inget studieinnehåll innan korrekt PIN-kod har angetts. Den här versionens PIN är **4826**. Upplåsningen sparas endast för den aktuella webbläsarfliken (`sessionStorage`). När fliken stängs krävs PIN igen. En låsknapp finns uppe till höger för att låsa sidan manuellt.

Efter fem felaktiga försök spärras nya försök i 30 sekunder. PIN-koden ligger inte i klartext i JavaScript utan jämförs som SHA-256-hash. Observera att detta är ett klientbaserat åtkomsthinder på GitHub Pages och inte samma sak som serverbaserad autentisering; en tekniskt kunnig person kan fortfarande kringgå det genom att inspektera källkoden.

V6.2 lägger till filen `pin-lock.js`. När du uppdaterar GitHub ska den ligga i repositoryts rot tillsammans med `index.html`, `app.js`, `module-v6.js` och övriga filer.
